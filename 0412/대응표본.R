...
# 20대 운동 전과 후 감소가 유의한 효과가 있는 지 대응표본으로 검증
library(dplyr)
# substr : 특정문자 추출
# 20대만 필터링해서 출력
# !is.na : 결측값이 아닌
# is.na : 결측값이면
data = exercise_data %>% filter(substr(Age,1,1) == '2' & !is.na(Post_Weight))
#View(data) -> 결측값 제거된 데이터
# result_20 = exercise_data %>% filter(Age >= 20 & Age <= 29)
# print(result_20)
